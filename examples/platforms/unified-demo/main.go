...
func main() {
	fmt.Println("🌟 Unified Platform Demo - All Platforms Together")
	fmt.Println("================================================")
	fmt.Println()
	...
	fmt.Println("📋 Step 1: Platform Registration Overview")
	fmt.Println("---------------------------------------")
	...
	fmt.Println("🚀 Step 2: Creating Unified Multi-Platform Hub")
	fmt.Println("--------------------------------------------")
	...
	if err != nil {
		log.Fatalf("❌ Failed to create unified hub: %v", err)
	}
	defer func() { _ = hub.Close(context.Background()) }()

	fmt.Println("✅ Unified hub created successfully!")
	fmt.Printf("   📱 Platforms configured: Feishu, Email, SMS, Discord\n")
	fmt.Printf("   🏗️  Architecture: Built-in + External platforms\n")
	fmt.Printf("   🔧 API: Consistent across all platforms\n")
	fmt.Println()

	ctx := context.Background()

	// Step 3: Cross-Platform Broadcasting
	fmt.Println("📡 Step 3: Cross-Platform Broadcasting")
	fmt.Println("------------------------------------")

	broadcastMsg := notifyhub.NewMessage("🌟 Unified Platform Broadcast").
	...
	normalMsg := notifyhub.NewMessage("📝 Regular Update").
	...
	alertMsg := notifyhub.NewAlert("⚠️  System Alert").
	...
	urgentMsg := notifyhub.NewUrgent("🚨 CRITICAL: Service Down").
	...
	for description, msg := range priorityMessages {
		...
		if err != nil {
			...
			fmt.Printf("✅ %s sent to %d platforms (Priority: %d)\n",
	...
	fmt.Println("🎨 Step 5: Platform-Specific Rich Content")
	fmt.Println("---------------------------------------")
			...
			"feishu_card": map[string]interface{}{
				"header": map[string]interface{}{
					"title": map[string]interface{}{
						"tag":     "plain_text",
						"content": "🌟 Unified Platform Status",
					},
					"template": "green",
				},
	...
	emailRichMsg := notifyhub.NewMessage("📊 Platform Status Report").
		WithBody(`
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="header">
        <h1>🌟 Unified Platform Status Report</h1>
	...
	discordRichMsg := notifyhub.NewMessage("Platform Integration Success").
		...
		WithPlatformData(map[string]interface{}{
			"discord_embeds": []map[string]interface{}{
				{
					"title":       "🌟 Unified Platform Achievement Unlocked!",
					"description": "All platforms successfully integrated and operational",
					"color":       0x00ff00, // Green
					"fields": []map[string]interface{}{
						{
							"name":   "🏗️ Architecture",
							"value":  "• Built-in platforms: Feishu, Email, SMS\n• External platforms: Discord\n• Unified API experience",
							"inline": false,
						},
						{
							"name":   "📊 Stats",
	...
	smsRichMsg := notifyhub.NewUrgent("Platform Status").
		WithPlatformData(map[string]interface{}{
			"sms_template": "🌟 All platforms ONLINE. {{platforms}} active. Success rate: {{rate}}%. Dashboard: {{url}}",
	...
	fmt.Println("🔧 Step 6: Unified Configuration Patterns")
	fmt.Println("---------------------------------------")
	...
	fmt.Println()
	fmt.Println("✅ Consistent patterns:")
	fmt.Println("   • Main function with required parameters")
	fmt.Println("   • Optional parameter functions")
	fmt.Println("   • Type-safe configuration")
	fmt.Println("   • Auto-registration mechanism")
	fmt.Println()

	// Step 7: Error Handling Across Platforms
	fmt.Println("🛡️  Step 7: Unified Error Handling")
	fmt.Println("--------------------------------")
	...
	if err != nil {
		...
		fmt.Printf("📊 Mixed results: %d successful, %d failed\n", receipt.Successful, receipt.Failed)
		fmt.Println("\nDetailed results:")
		for _, result := range receipt.Results {
			status := "✅"
			errorInfo := ""
	...
	fmt.Println("📈 Step 8: Performance Metrics")
	fmt.Println("----------------------------")
	...
	if err != nil {
		...
		fmt.Printf("✅ Performance test completed in %dms\n", totalTime.Milliseconds())
	...
	fmt.Println("🏗️  Step 9: Architecture Summary")
	fmt.Println("------------------------------")
	fmt.Println("The unified platform architecture provides:")
	fmt.Println()
	fmt.Println("✅ TRUE EXTERNAL EXTENSIBILITY:")
	fmt.Println("   • Discord created without modifying core")
	fmt.Println("   • External platforms = built-in quality")
	fmt.Println("   • Community ecosystem enabled")
	fmt.Println()
	fmt.Println("✅ UNIFIED DEVELOPER EXPERIENCE:")
	fmt.Println("   • Consistent API patterns")
	fmt.Println("   • Same configuration approach")
	fmt.Println("   • Unified error handling")
	fmt.Println()
	fmt.Println("✅ OPERATIONAL EXCELLENCE:")
	fmt.Println("   • Auto-registration system")
	fmt.Println("   • Performance monitoring")
	fmt.Println("   • Graceful error handling")
	fmt.Println()
	fmt.Println("✅ PLATFORM CAPABILITIES:")
	fmt.Println("   • Rich content per platform")
	fmt.Println("   • Platform-specific features")
	fmt.Println("   • Cross-platform broadcasting")
	fmt.Println()

	fmt.Println("🌟 Unified Platform Demo Complete!")
	fmt.Println()
	fmt.Println("This demo proved that the unified architecture successfully:")
	fmt.Println("• Integrates all platforms seamlessly")
	fmt.Println("• Provides consistent developer experience")
	fmt.Println("• Enables true external extensibility")
	fmt.Println("• Maintains platform-specific rich features")
	fmt.Println("• Handles errors gracefully across platforms")
	fmt.Println()
	fmt.Println("The original architecture problem is SOLVED! 🎉")
}
