...
func main() {
	...
	_, duration, err := sendWithMetrics(hub, ctx, metricsMsg)
	if err != nil {
	...
	_, err = sendWithRetry(hub, ctx, retryMsg, 3)
	if err != nil {
	...
	_, err = sendWithCircuitBreaker(hub, ctx, cbMsg)
	if err != nil {
	...
	// Execute chain
	_, err = chain.Execute(hub, ctx, chainMsg)
	if err != nil {
