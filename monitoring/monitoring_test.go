...
func BenchmarkRecordSend(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		metrics.RecordSend("benchmark", true, time.Millisecond, "")
	}
}

func BenchmarkRecordHealth(b *testing.B) {
	metrics := NewMetrics()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		metrics.RecordHealth("benchmark", true)
	}
}

func BenchmarkGetSuccessRate(b *testing.B) {
	metrics := NewMetrics()
	// Prepare some data
	for i := 0; i < 1000; i++ {
		metrics.RecordSend("benchmark", i%2 == 0, time.Millisecond, "")
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkConcurrentRecordSend(b *testing.B) {
	metrics := NewMetrics()
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			metrics.RecordSend("concurrent", true, time.Millisecond, "")
		}
	})
}